...
# global variable
initial_cmd = 'a' # In the manual mode, user can get out of the loop when he pushes "p." Here, we set the value as "a" in initial state 
dis_th = 0.5 # range as arriving at the goal 
obj_th = 1.0 # threshold for collision avoidance 
time_th = rospy.Duration(60) # 1 minute is the maximum time for the car to reach the goal
vel_msg = Twist() # defining vel_msg 
...
def get_user_input():
    # getting and checking user input and asking for another user input if it is ineffective
    while True:
        try:
...
def assisted_driving(msg):
    # in each step, car collects the information on the distance between a car and an object with laserscan topic
    # and decide the speed in assisted mode
